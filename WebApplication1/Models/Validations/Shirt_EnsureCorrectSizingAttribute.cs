  ...
  public class Shirt_EnsureCorrectSizingAttribute : ValidationAttribute
  {
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
      ...
      if (shirt != null && !string.IsNullOrWhiteSpace(shirt.Gender))
      {
        ...
        {
          return new ValidationResult("for male's shirts, the size has to be greater or equal to 8 ");
        }
        else if (shirt.Gender.Equals("female", StringComparison.OrdinalIgnoreCase) && shirt.Size < 6)
        {
          return new ValidationResult("for female's shirts, the size has to be greater or equal to 6 ");
        }
      }
